# -*- coding: utf-8 -*-
"""run_atis_bert.py - 使用 train/dev/test 数据，预测阶段仅替换 text 中实际出现的变量"""
...
with open("query_train.json", encoding="utf-8") as f:
    train_entries = json.load(f)
with open("query_dev.json", encoding="utf-8") as f:
    dev_entries = json.load(f)
with open("query_test.json", encoding="utf-8") as f:
    test_entries = json.load(f)
...
def prepare_data(entries):
    ...
    for entry in entries:
        tmpl = entry["sql"]
        tid = template2id.get(tmpl, template2id["UNK"])   # 如果没见过，用 -1 占位
        # if tid == -1:
        #     continue  # 或者你可以选择收集这些样本作为 "unknown"
...
tok_model = BertForTokenClassification.from_pretrained("bert-base-uncased", num_labels=len(label2id), id2label=id2label, label2id=label2id).to(DEVICE)

args_cls = TrainingArguments("cls_out", num_train_epochs=1, per_device_train_batch_size=16, learning_rate=5e-5, logging_steps=100, evaluation_strategy="epoch", save_strategy="epoch")
args_tok = TrainingArguments("tok_out", num_train_epochs=1, per_device_train_batch_size=16, learning_rate=3e-5, logging_steps=100, evaluation_strategy="epoch", save_strategy="epoch")

print("\n>>> 训练模板分类器 ...")
Trainer(model=cls_model, args=args_cls, train_dataset=train_ds_cls, eval_dataset=dev_ds_cls).train()

print("\n>>> 训练槽位标注器 ...")
Trainer(model=tok_model, args=args_tok, train_dataset=train_ds_tok, eval_dataset=dev_ds_tok, data_collator=DataCollatorForTokenClassification(TOKENIZER)).train()
...
with torch.no_grad():
    logits_cls = cls_model(**{k: enc_test[k] for k in ["input_ids", "attention_mask"]}).logits
    pred_tids = logits_cls.argmax(dim=1).tolist()

    logits_tok = tok_model(**{k: enc_test[k] for k in ["input_ids", "attention_mask"]}).logits
