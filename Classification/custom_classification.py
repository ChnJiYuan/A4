# -*- coding: utf-8 -*-
"""custom_classification.py - 使用自定义模型进行SQL模板分类和槽位标注"""
...
class CustomClassificationModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_labels):
        ...
        self.transformer = CustomTransformer(embedding_dim)
        self.classifier = nn.Linear(embedding_dim, num_labels)
        
    def forward(self, input_ids, attention_mask=None):
        x = self.embedding(input_ids)
        x = x.permute(1, 0, 2)  # 转换为transformer期望的格式
        x = self.transformer(x, mask=~attention_mask.bool() if attention_mask is not None else None)
...
cls_model = CustomClassificationModel(VOCAB_SIZE, EMBEDDING_DIM, NUM_TEMPLATES).to(DEVICE)
tok_model = CustomTokenClassificationModel(VOCAB_SIZE, EMBEDDING_DIM, len(label2id)).to(DEVICE)
...
print("\n>>> 训练槽位标注器 ...")
train_model(tok_model, train_loader_tok, dev_loader_tok, num_epochs=20, learning_rate=1e-4, is_token_classification=True)
